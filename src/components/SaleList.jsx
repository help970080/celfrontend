            ...
            <table className="sale-table">
                ...
                <tbody>
                    {sales.map(sale => {
                        ...
                        const productListText = (sale?.saleItems?.length > 0) ? sale.saleItems.map(item => `${item.quantity || '?'}x ${item.product?.name || 'Producto no encontrado'}`).join(', ') : 'N/A';
                                ...
                                <td>
                                    {sale.isCredit && hasPermission(['super_admin', 'regular_admin', 'sales_admin']) ? (
                                        <div className="assignment-cell">
                                            ...
                                            <select defaultValue="" onChange={(e) => handleAssignCollector(sale.id, e.target.value)} disabled={assigningId === sale.id}>
                                                <option value="" disabled>{assigningId === sale.id ? 'Guardando...' : 'Cambiar...'}</option>
